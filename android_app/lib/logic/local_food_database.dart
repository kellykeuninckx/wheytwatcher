...
/// Poort van `LocalFoodDatabase.swift`: vaste lijst met veelgebruikte basisproducten
/// zonder barcode, doorzocht vóór een eventuele netwerkzoekopdracht (nog niet geport).
class LocalFoodDatabase {
  LocalFoodDatabase._();

  static const List<LocalFoodItem> items = [
    // Fruit
  ...
  /// Zoekt basisproducten op naam, hoofdlettterongevoelig. (Accentongevoelig zoals de Swift-
  /// versie volgt later — de huidige lijst bevat toevallig geen namen met diakrieten.)
  static List<LocalFoodItem> search(String query) {
    final normalized = query.trim().toLowerCase();
    if (normalized.isEmpty) return const [];
    return items.where((item) => item.name.toLowerCase().contains(normalized)).toList();
  }
}
